...
class CheckoutController extends Controller
{
    ...
    public function checkout(CheckoutRequest $request)
    {
        ...
        try {
            ...
            // Check payment_method FE gửi lên và trả về dữ liệu tương ứng (không tạo URL API mới).
            if ($order->payment_method === 'vnpay') {
                $paymentUrl = $this->buildVnpayRedirectUrl($order, $request);
    ...
    private function buildVnpayRedirectUrl(Order $order, \Illuminate\Http\Request $request): string
    {
        ...
        foreach ($inputData as $key => $value) {
            $hashData .= $hashData === '' ? urlencode($key).'='.urlencode($value) : '&'.urlencode($key).'='.urlencode($value);
            $query .= urlencode($key).'='.urlencode($value).'&';
        }
